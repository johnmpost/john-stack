...
/** @internal */
export const getMessage = e => AST.getMessageAnnotation(e.expected).pipe(Option.map(annotation => annotation(e.actual)), Option.orElse(() => e.message), Option.getOrElse(() => `Expected ${formatExpected(e.expected)}, actual ${formatActual(e.actual)}`));
const go = e => {
  switch (e._tag) {
    case "Type":
