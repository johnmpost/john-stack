...
const done = exit => {
  return {
    ...
    exit,
    [Hash.symbol]() {
      return (0, _Function.pipe)(Hash.hash(STMStateSymbolKey), Hash.combine(Hash.hash(OpCodes.OP_DONE)), Hash.combine(Hash.hash(exit)));
    },
    [Equal.symbol](that) {
...
const interrupted = exports.interrupted = {
  ...
  [Hash.symbol]() {
    return (0, _Function.pipe)(Hash.hash(STMStateSymbolKey), Hash.combine(Hash.hash(OpCodes.OP_INTERRUPTED)), Hash.combine(Hash.hash("interrupted")));
  },
  [Equal.symbol](that) {
...
const running = exports.running = {
  ...
  [Hash.symbol]() {
    return (0, _Function.pipe)(Hash.hash(STMStateSymbolKey), Hash.combine(Hash.hash(OpCodes.OP_RUNNING)), Hash.combine(Hash.hash("running")));
  },
  [Equal.symbol](that) {
